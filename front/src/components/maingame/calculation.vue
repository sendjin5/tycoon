    ...
    <div style="text-align: center">
      <div class="calculation-paper">
        <div class="calculation-title">
          <p style="color: white; font-size: 24px; margin: 0.8vh 0">
            {{ revenue.salesDay }}일차 매출 정산
          </p>
        </div>
        <div
          style="
        ...
        >
          <div>
            ...
            <div class="block-right">
              <h3>{{ calcul + revenue.disposePrice }}원</h3>
            </div>
          </div>
          <div>
            <div class="block-left"><p>판매 수익</p></div>
            <div class="block-right">
              <p>{{ revenue.salesMount }}원</p>
            </div>
          </div>
          <div>
            <div v-show="revenue.qeezeYN == 'Y'" class="block-left">
              <p style="position: relative; left: 2vw">*퀴즈 혜택</p>
            </div>
            <div v-show="revenue.qeezeYN == 'Y'" class="block-right">
              <p>{{ reward }}</p>
            </div>
          </div>
          <div>
            <div v-show="revenue.feverYN == 'Y'" class="block-left">
              <p style="position: relative; left: 2vw">*FEVER DAY</p>
            </div>
        ...
        >
          <div>
            ...
            <div class="block-right">
              <h3>{{ revenue.orderPrice - 20000 }}원</h3>
            </div>
          </div>
          <div>
            <div class="block-left"><p>발주 비용</p></div>
            <div class="block-right">
              <p>{{ revenue.orderPrice }}원</p>
            </div>
          </div>
          <div>
        ...
        <div style="height: 3vh">
          <div>
            ...
            <div class="block-right">
              <h3>{{ calcul + revenue.disposePrice + revenue.orderPrice - 20000 }}원</h3>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    convEnd() {
      // 게임정보, 수익, 창고 업데이트 하기
      fetch(__apiUrl__ + "/spring/maingame/gameend", {
        method: "POST",
        headers: {
  ...
  mounted() {
    ...
    this.revenue.cash =
      this.revenue.cash * 1 +
      this.calcul +
      this.revenue.disposePrice +
      this.revenue.orderPrice -
      20000;
