<template>
    ...
    <div id="cartzone" style="width: 38vw; display: flex; overflow-x: auto">
      ...
      <div v-for="c in cart.cart" v-show="c.orderQuantity > 0" @click="revertprod($event)">
        ...
        <img
          ...
          :id="`cart${c.goodsNo}${c.expDate == 1 ? '_50' : ''}`"
          :name="c.goodsName"
        /><br />
  ...
  <div
    v-show="timeleft == 0 || noclick"
    style="width: 100vw; height: 30.5vh; position: relative; bottom: 30.5vh"
  ></div>
</template>
<script>
  ...
  methods: {
    revertprod(e) {
      ...
      if (prod != null) {
        const target = {
          ...this.cart.cart.find(
            (c) => "cart" + c.goodsNo + (c.expDate == 1 ? "_50" : "") == prod.id
          ),
          sell: 0,
        };
