      ...
      if (p50 != null) {
        while (p50.orderQuantity > 0) {
          const prod = this.product.find(
            (pro) => pro.goodsName == p50.goodsName && pro.expDate == 1 && pro.orderQuantity > 0
          );
        ...
        while (p.orderQuantity > 0) {
          ...
          while (prod == null && i <= expDate) {
            prod = this.product.find(
              (pro) =>
                pro.goodsName == p.goodsName && pro.expDate == expDate && pro.orderQuantity > 0
            );
          }
